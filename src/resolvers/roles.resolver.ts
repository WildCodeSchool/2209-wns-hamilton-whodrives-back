...
Mutation: {
            createRole: async (_: any, args: {name :string}, { res }: ExpressContext) => {
                const { name} = args;
                let Role = await new RolesController().addRole({ name });
                return Role
            },
    
            updateRole: async (_: any, args: {id: number,name :string}, { res }: ExpressContext) => {   
                const { id, name} = args;
                let Role = await new RolesController().updateRole({ id, name });
                return Role
            },
    
            deleteRole: async (_: any, args: {id: number}, { res }: ExpressContext) => {
                const { id } = args;
