...
export default {
    ...
    Mutation: {
        createCar: async (_: any, args: MutationCreateCarArgs, { res }: ExpressContext) => {
            const { id, seat, modelId, optionId } = args;
            let car = await new CarController().addCar({ id, seat, modelId, optionId });
            return car
        },

        updateCar: async (_: any, args: MutationUpdateCarArgs, { res }: ExpressContext) => {   
            const { id, seat, modelId, optionId } = args;
            let car = await new CarController().updateCar({ id, seat, modelId, optionId });
            return car
        },

        deleteCar: async (_: any, args: MutationDeleteCarArgs, { res }: ExpressContext) => {
            const { id } = args;
