import { MutationCreateRatingArgs, MutationDeleteRatingArgs, MutationUpdateRatingArgs } from '@/graphgen';
...
Mutation: {
            createRating: async (_: any, args: MutationCreateRatingArgs, { res }: ExpressContext) => {
                const { note, content} = args;
                let Rating = await new RatingController().addRating({ note, content });
                return Rating
            },
    
            updateRating: async (_: any, args: MutationUpdateRatingArgs, { res }: ExpressContext) => {   
                const { id, note ,content} = args;
                let Rating = await new RatingController().updateRating({ id, note ,content});
                return Rating
            },
    
            deleteRating: async (_: any, args: MutationDeleteRatingArgs, { res }: ExpressContext) => {
                const { id } = args;
